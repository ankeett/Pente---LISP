...
(defun calculate-score (result)
  (let* ((board (first result))
    ...
    ;; Now you can access and deduce information from the elements
    ;; (format t "Winner: ~a~%" winnerType)
    ;; (format t "Opponent: ~a~%" opponentType)
    ;; (format t "Winner Score: ~d~%" (+ winnerCapture isFiveinarow))
    ;; (format t "Computer Score: ~d~%" opponentCapture)
    
    ;;call four in a row here
    ;;
...
(defun tournament(humanScore computerScore)
  ...
  ;;   (start-round "Human" "Computer"))

  ;;   ((< humanScore computerScore)
  ;;   (start-round "Computer" "Human"))

  ;;   (t
  ;;   (let* ((playerList (start-game))
  ;;           (result (start-round (first playerList) (second playerList))))
  ;;     (print (calculate-score result))
  ...
  ((> humanScore computerScore)
   (let* ((result (start-round "Human" "Computer")))
     (let* ((scores (calculate-score result)))
       ...
       (format t "Human Scores: ~a~%" (+ (first scores) humanScore))
       (format t "Computer Scores: ~a~%" (+ (second scores) computerScore))
       (format t "Continue the tournament? (yes/no): ~%") ; Moved this line here
